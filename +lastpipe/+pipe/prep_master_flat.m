...
% Example: lastpipe.pipe.prep_master_flat('Date',celestial.time.julday([16 9 2020]));
...
% Read master dark image
% In the future consider treat ExpTime...
[~,~,Dark] = lastpipe.db.latest_calib_image('NeededJD',InPar.Date,...
                                            'DataBaseName','table','Type','dark',...
...
if ~isempty(InPar.SubCCDSEC) && ~isempty(InPar.NewNoOverlap)
    AllNamesS=lastpipe.util.save_product(Flat,...
                    'Config_camera',Config.Camera,...
                    'Date',MidJD,...
                    'Fields',InPar.SaveProd,...
                    'Type','flat','Level','proc','SubLevel','n',...
